...
class OrderController extends Controller
{
    ...
    public function store(Request $request, $set_paid = false)
    {
        ...
        // return Carbon::now();
        $carbon_req_date = Carbon::parse($request->order_required_date);

        if ($carbon_req_date <= Carbon::now()) {
            return $this->returnError('301', 'يجب ان يكون تاريخ التوصيل اكبر من تاريخ اليوم');
        }

        $day = WeekDays::where(['id' => $carbon_req_date->dayOfWeek, 'active' => 1])->first();
        if (!$day) {
        ...
        if (
            ...
            Carbon::parse($request->required_time) <= Carbon::parse($day->end_time)
        ) {
            // return 'open';
        } else {
        ...
        foreach ($seller_products as $seller_product) {
            $order = new Order;
            $order->order_required_date = $request->order_required_date;
            $order->required_time = $request->required_time;
        ...
        return response()->json([
            ...
            'orders_id' => implode(",", $new_orders_id),
            'result' => true,

...
if (!function_exists('home_discounted_price')) {
    ...
    {
        $lowest_price = $product->main_price;
        $highest_price = $product->main_price;
...
if (!function_exists('home_discounted_base_price')) {
    function home_discounted_base_price($product, $formatted = true)
    {
        ...
        $price += $tax;
        // $price = $product->zones->where('zone_id' , 1)->first()->cost;
        ...
        if (($asset = Upload::find($id)) != null) {
            if ($asset->external_link == null) {
                if (file_exists(public_path($asset->file_name))) {
                    ...
                    return my_asset('assets/img/placeholder.jpg');
                }
            } else {
...
if (!function_exists('seller_package_validity_check')) {
    ...
    {
        $user = $user_id == null ? User::find(Auth::user()->id) : User::find($user_id);
        $shop = $user->shop;

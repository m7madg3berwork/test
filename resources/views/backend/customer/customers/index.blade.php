        ...
        <div class="card-header row gutters-5">
            ...
            <div class="dropdown mb-2 mb-md-0">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                    ...
                    <a class="dropdown-item" href="#" onclick="bulk_send_money()">{{translate('Add To Wallet')}}</a>
                </div>
            </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="form-check">
                                <input id="changeActiveUser{{ $user->id }}" onchange="changeActive({{ $user->id }})"
                                    class="form-check-input" type="checkbox" {{ $user->active == 1 ? '
                                checked ' : '' }}>
                            </div>
                        </td>
                        <td>{{$user->email}}</td>
                        ...
                        <td>
                            ...
                            {{$user->orders_count}}
                            @else
                            <span class="btn btn-danger btn-sm" style="font-size:12px;padding: 3px 5px;cursor: none;">
                                {{translate('No Orders')}}
                            </span>
                            @endif
                        </td>
                        ...
                        <td><span class="btn btn-danger btn-sm"
                                style="font-size:13px; color: white;padding: 3px 5px;cursor: none;">{{translate('Blocked')}}</span>
                        </td>
                        @else
                        ...
                        <td class="text-right">
                            <a href="#" class="btn btn-soft-secondary btn-icon btn-circle btn-sm confirm-add-to-wallet"
                                data-name="{{ $user->name ?? '' }}" data-id="{{ $user->id ?? '' }}"
                                title="{{ translate('Add To Wallet') }}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>{{translate('Do you really want to block this Customer?')}}</p>
            </div>
...
<div class="modal fade" id="confirm-unban">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>{{translate('Do you really want to active this Customer?')}}</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light" data-dismiss="modal">{{translate('Cancel')}}</button>
...
@include('modals.add_to_wallet')
@include('modals.add_broadcast_wallet')
@endsection
...
<script type="text/javascript">
    $(document).on("change", ".check-all", function () {
        ...
        $(document).on("click", "#sendBroadCast", function () {
            var bodyFormData = new FormData($('#sort_customers')[0]);
            bodyFormData.append("amount", $("#amount").val())
            bodyFormData.append("comment", $("#comment").val())
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: "{{route('customers.add-wallet-broadcast')}}",
                type: 'POST',
                data: bodyFormData,
                cache: false,
                contentType: false,
        ...
        function bulk_send_money() {
            $("#add-broadcast-wallet-modal").modal("show");
        }
</script>
@endsection
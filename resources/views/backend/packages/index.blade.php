            ...
            <div class="card">
                <div class="card-body text-center">
                    ...
                    <p class="fs-15">{{translate('Qty') }}:
                        <span class="text-bold">{{$package->qty ?? ''}}</span>
                    </p>
                    <p class="fs-15">{{translate('Customer type') }}:
                        <span class="text-bold">{{$package->customer_type ?? ''}}</span>
                    </p>
                    <div class="mar-top">
                        <a href="{{route('packages.edit', ['id'=>$package->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" class="btn btn-sm btn-info">{{translate('Edit')}}</a>
                        <a href="#" data-href="{{route('packages.destroy', $package->id)}}" class="btn btn-sm btn-danger confirm-delete" >{{translate('Delete')}}</a>
                    </div>
                </div>
            </div>
        </div>

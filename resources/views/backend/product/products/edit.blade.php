            ...
            <div class="col-lg-8">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <input type="text" class="form-control" name="name"
                                    placeholder="{{translate('Product Nsssame')}}"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <input type="number" lang="en" class="form-control" name="max_qty"
                                    value="{{$product->max_qty}}" min="1" required>
                            </div>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <select class="form-control aiz-selectpicker" name="video_provider" id="video_provider">
                                    <option value="youtube" <?php if ($product->video_provider == 'youtube') echo
                                        "selected"; ?> >{{translate('Youtube')}}</option>
                                    <option value="dailymotion" <?php if ($product->video_provider == 'dailymotion')
                                        echo "selected"; ?> >{{translate('Dailymotion')}}</option>
                                    <option value="vimeo" <?php if ($product->video_provider == 'vimeo') echo
                                        "selected"; ?> >{{translate('Vimeo')}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            <label class="col-sm-3 col-from-label" for="start_date">{{translate('Discount Date
                                Range')}}</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control aiz-date-range" @if($product->discount_start_date
                                && $product->discount_end_date) value="{{ $start_date.' to '.$end_date }}"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-3">
                                <select class="form-control aiz-selectpicker" name="discount_type" required>
                                    <option value="amount" <?php if ($product->discount_type == 'amount') echo
                                        "selected"; ?> >{{translate('Flat')}}</option>
                                    <option value="percent" <?php if ($product->discount_type == 'percent') echo
                                        "selected"; ?> >{{translate('Percent')}}</option>
                                </select>
                            </div>
                        </div>
                ...
                {{-- <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input type="number" name="return_policy" value="{{ $product->return_policy }}" id=""
                                    placeholder="{{ translate('Returned Policy') }}" class="form-control">
                        ...
                        <div class="form-group row">
                            <div class="col-md-12">
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input type="checkbox" name="featured" value="1" @if($product->featured ==
                                            1) checked @endif>
                                            <span></span>
                                        </label>
                                    </div>
...
<script type="text/javascript">
    ...
    const items = @json($statesSelected);
    ...
    function getSelectedStates() {
        if (items.length > 0)
            for (let i = 0; i < items.length; i++) {
                var html = '<div class="row mb-3">'
                var data =  @json($statesSelected);
                var states =  @json($states);
                var optionsStateHtml = states.map(function (opt) {
                    var selected = '';
                    if (data[i].state_id == opt.id)
                        selected = 'selected'
                    return '<option value="' + opt.id + '" '+ selected +'>' + opt.name + '</option>';
                }).join('');
                var optionsCostHtml = '<input name="states[' + i + '][cost]" type="number" value="'+data[i].cost+'" placeholder="{{ translate('Price') }}" class="form-control" />';
                var optionsDeleteHtml = '<a href="javascript:void(0);" id="removeItem" class="btn btn-soft-primary btn-icon btn-circle btn-sm"><i class="las la-trash"></i></a>';
                var optionsQtyHtml = '<input name="states[' + i + '][qty]" type="number"  value="'+data[i].qty+'" placeholder="{{ translate('Qty') }}" class="stateQty form-control" oninput="calcQty()" />';
                html += '<div class="col-4">'
                html += '<select name="states[' + i + '][state_id]" class="form-control aiz-selectpicker" data-placeholder="Choose ..." data-live-search="true">'
                html += '<option>Select City</option>'
                html += optionsStateHtml

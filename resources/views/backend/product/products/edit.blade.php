            ...
            <div class="row gutters-5">
                <div class="col-lg-8">
                                    ...
                                    <select class="form-control aiz-selectpicker" name="brand_id" id="brand_id"
                                        ...
                                        @foreach (\App\Models\Brand::all() as $brand)
                                            <option value="{{ $brand->id }}"
                                                    @if($product->brand_id == $brand->id) selected @endif>{{ $brand->getTranslation('name') }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="form-group row">
                                <label class="col-lg-3 col-from-label">{{translate('Default price')}}</label>
                                <div class="col-lg-6">
                                    <input type="text" placeholder="{{translate('Default price')}}" name="unit_price"
                            ...
                            <div class="card border-0">
                                <div class="card-header border-0">
                                    <h5 class="mb-0 h6">{{ translate('Unit Price Per Zones') }}</h5>
                                </div>
                                <div class="card-body border-0">
                                    <div class="form-group mb-3">
                                        <label for="name">
                                            <a href="javascript:void(0);" id="pushZone" class=" btn-sm btn btn-outline-danger"><i class="fas fa-plus"></i>{{ translate('Add New') }}</a>
                                        </label>
                                        <div class="zoneItems">
                                        </div>
                                    </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control aiz-date-range"
                                           ...
                                           @endif name="date_range" placeholder="{{translate('Select Date')}}"
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5 class="mb-0 h6">{{ translate('Return Policy') }}</h5>
                        </div>
                        <div class="card-body">
                            <div class="form-group row">
                                <label class="col-md-3 col-from-label">{{ translate('Return Policy') }}</label>
                                <div class="col-md-8">
                                    <input type="number" name="return_policy" value="{{ $product->return_policy }}" id="" placeholder="{{ translate('Returned Policy') }}" class="form-control">
                                    {{--                                    <textarea class="aiz-text-editor"--}}
{{--                                              name="return_policy">{{ $product->getTranslation('return_policy', $lang) }}</textarea>--}}
                                </div>
                            </div>
                        </div>
                    </div>
...
{{--                            <div class="form-group mb-3">--}}
{{--                                <label for="name">--}}
{{--                                    {{ translate('Select Zones') }} <a href="javascript:void(0);"--}}
{{--                                                                       id="pushZone">{{ translate('Add New') }}</a>--}}
{{--                                </label>--}}
{{--                                <div class="zoneItems">--}}
{{--                                </div>--}}
{{--                            </div>--}}
            ...
            if (items.length > 0)
                for (let i = 0; i < items.length; i++) {
                    ...
                    var optionsZoneHtml = options.map(function (opt) {
                        return '<option value="' + opt.id + '">' + opt.name_city + '</option>';
                    }).join('');
                    var optionsCostHtml = '<input name="zones[' + i + '][cost]" type="number" placeholder="{{ translate('Price') }}" class="form-control" />';
                    var optionsQtyHtml = '<input name="zones[' + i + '][qty]" type="number" placeholder="{{ translate('Qty') }}" class="form-control" />';
                    ...
                    html += '<select name="zones[' + i + '][zone_id]" class="form-control aiz-selectpicker" data-placeholder="Choose ..." data-live-search="true">'
        ...
        function getSelectedZones() {
            if (items.length > 0)
                for (let i = 0; i < items.length; i++) {
                    ...
                    var optionsZoneHtml = zones.map(function (opt) {
                        var selected = '';
                        if (data[i].zone_id == opt.id)
                            selected = 'selected'
                        return '<option value="' + opt.id + '" '+ selected +'>' + opt.name_city + '</option>';
                    }).join('');
                    var optionsCostHtml = '<input name="zones[' + i + '][cost]" type="number" value="'+data[i].cost+'" placeholder="{{ translate('Price') }}" class="form-control" />';
                    var optionsDeleteHtml = '<a href="javascript:void(0);" id="removeItem" class="btn btn-soft-primary btn-icon btn-circle btn-sm"><i class="las la-trash"></i></a>';
                    var optionsQtyHtml = '<input name="zones[' + i + '][qty]" type="number"  value="'+data[i].qty+'" placeholder="{{ translate('Qty') }}" class="form-control" />';
                    html += '<div class="col-4">'
                    html += '<select name="zones[' + i + '][zone_id]" class="form-control aiz-selectpicker" data-placeholder="Choose ..." data-live-search="true">'
                    html += '<option>Select Zone</option>'
                    html += optionsZoneHtml
